            ...
            <div class="col-md-12">
              <div class="card">
                <div class="card-header">
                  <h3 class="card-title" style="font-size : 1.4em;">Result List</h3>
                ...
                <div class="card-body p-2 table-scrollable-x">
                  <table id="{{ default(datatableId, "") }}" class="table table-bordered table-sm table-stripped table-hover" cellspacing="0" width="100%">
                    ...
                    <tbody>
                      {% set idx=0 %}
                      {% for ent in lisorders %}
                      {% set idx=idx+1 %}
                      <tr tbs_data_msg_id="{{ent.headerId}}" >
                        <td>
                          {% if 0>1 %}
                          <!--a href="#" onclick="showFormDetail('{{ent.headerId}}');" data-bs-toggle="tooltip" data-bs-placement="top" title="View details"><i class="fas fa-table"></i></a-->
                          <!--a href="#" onclick="showFormDelete('{{ent.headerId}}');"  data-bs-toggle="tooltip" data-bs-placement="top" title="Delete data"><i class="fas fa-trash"></i></a-->
                          {% endif %}

                        </td>
                        <th scope="row">{{ idx }}</th>
                        <td>{{ ent.headerId }}</td>
                        <td>{{ ent.patientName }}</td>
                        <td>{{ ent.msgDatetime }}</td>
                        <td>{{ ent.specId }}</td>
                        <td>{{ ent.specType }}</td>
                        <td>{{ ent.instrument }}</td>
                     </tr>
                      {% endfor %}
                    </tbody>
                  </table>
...
{% block pageJS %}
    <script>
      function getResultList(msgId, handler) {
         let url = TBS.baseUrl + '/api/lis/lis2a_result_list/' + msgId;
      ...
      function onGetEntries(filterVal='') {
         ...
         if (filterVal.length > 0)
            location.href = '{{ pageBaseUrl }}/lis/testdev_lis1a?startdate=' + startDate + '&enddate=' + endDate  + '&filter=' + filterVal;
         else
            location.href = '{{ pageBaseUrl }}/lis/testdev_lis1a?startdate=' + startDate + '&enddate=' + endDate;
      }

      function isResultImageField(res)
      {
         if (  res.testCode == "WBC Histogram. Total"
            || res.testCode == "WBC Histogram. BMP"
            || res.testCode == "RBC Histogram. Left Line"
            || res.testCode == "RBC Histogram. Right Line"
            || res.testCode == "RBC Histogram. Total"
            || res.testCode == "RBC Histogram. BMP"
            || res.testCode == "PLT Histogram. Left Line"
            || res.testCode == "PLT Histogram. Right Line"
            || res.testCode == "PLT Histogram. Total"
            || res.testCode == "PLT Histogram. BMP"
            || res.testCode == "WBC DIFF Scattergram. Fsc dimension"
            || res.testCode == "WBC DIFF Scattergram. Ssc dimension"
            || res.testCode == "WBC DIFF Scattergram. BMP"
            || res.testCode == "Baso Scattergram. Fsc dimension"
            || res.testCode == "Baso Scattergram. Ssc dimension"
            || res.testCode == "Baso Scattergram. BMP" )
         {
            return true;
         }
         return false;
      ...
      window.addEventListener('DOMContentLoaded', event => {
         ...
         {% if datatableId %}
            ...
            table.on('click', 'tbody td:not(:last-child)', function (e) {
               var currentRow = this.parentElement;
               let msgIdAttrib = currentRow.getAttribute("tbs_data_msg_id");
               ...
               if (processedAttrib == null) {
                  if (msgIdAttrib != null) {
                     getResultList(msgIdAttrib, function (data) {
                        ...
                        for (let res of data.result)
                        {
                           if ( ! isResultImageField(res) )
                           {
                              ...
                              // one time check only
                              if (!hasImageField)
                                 hasImageField = (res.binaryValue == 1);
                           }
                        }
                        ...
                        if (hasImageField)
                        {
                           colInnerHtml +=
                                    `<tr>
                                       <td colspan="6">
                                          <div class="row" id="res_image_row">`;

                           for (let res of data.result)
                           {
                              if (res.binaryValue == 1 && res.binaryEncoding == "Base64")
                              {
                                 var resImageName = "";
                                 if (res.testCode == "WBC Histogram. BMP")
                                    resImageName = "WBC Histogram";
                                 else if (res.testCode == "RBC Histogram. BMP")
                                    resImageName = "RBC Histogram";
                                 else if (res.testCode == "PLT Histogram. BMP")
                                    resImageName = "PLT Histogram";
                                 else if (res.testCode == "WBC DIFF Scattergram. BMP")
                                    resImageName = "WBC DIFF Scattergram";
                                 else if (res.testCode == "Baso Scattergram. BMP")
                                    resImageName = "Baso Scattergram";

                                 colInnerHtml +=
                                    `       <div class="col">
                                                <div class="card">
                                                   <div class="card-header">${resImageName}</div>
                                                   <div card-body>
                                                      <div class="d-flex justify-content-center align-items-center p-2">
                                                         <img src="data:image/png;base64, ${res.binaryData}" alt="Base64 Image" onclick="openModal(this)">
                                                      </div>
                                                   </div>
                                                </div> <!-- /card -->
                                             </div>`;
