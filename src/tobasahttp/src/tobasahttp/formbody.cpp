...
void FormBody::parse(const std::string& text)
{
   if (text.length() == 0)
      return;
...
std::optional<std::string> FormBody::get(const std::string& name) const
{
   for (auto f: _fields)
   {
      if ( f->name() == name)
...
std::optional<FieldPtr> FormBody::get(int32_t position) const
{
