...
DirUih500Record::DirUih500Record(const std::string& lisString)
   : lis::Record(lisString) {}

DirUih500Record::~DirUih500Record()
{
   _lisFields.clear();
}

bool DirUih500Record::fromString(const std::string& lisString)
{
   ...
   for (i = 0; i < array.size() - 1; i++)
   {
      ...
      if (!line.empty())
      {
         ...
         _lisFields.push_back(new DirUih500Fields(static_cast<int>(i), name_, field_, value_, rawdata_));
      }
   }
